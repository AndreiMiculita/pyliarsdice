...
class Model(object):
    ...
    mas = 2.0  # maximum spreading (:mas; default: 2.0)
    ...
    def add_encounter(self, chunk):
        ...
        if self.dm[chunk_idx].slots != chunk.slots:
            raise ValueError(
                f"Trying to add an encounter to a chunk with the same name ({chunk.name}) but different slots and/or "
                f"slot values")
    ...
    def get_activation_no_noise(self, chunk):
        ...
        baselevel_activation = math.log(
            sum([(self.time - encounter) ** -self.d for encounter in chunk.encounters if encounter < self.time]))
    ...
    def get_spreading_activation_from_goal(self, chunk):
        ...
        if type(self.goal) is Chunk:
            ...
            for value in self.goal.slots.values():
                ...
                if ch1 is not None and value in chunk.slots.values() and ch1.fan > 0:
                    spreading += max(0, int(self.mas - math.log(ch1.fan)))

        # TODO: This variable might be referenced before assignment
        if total_slots == 0:
            return 0
    ...
    def retrieve_partial(self, chunk, trace=False):
        ...
        for ch in self.dm:
            ...
            if penalty is not None and act + penalty > bestActivation:
                bestMatch = ch
                bestActivation = act + penalty
            if trace and penalty is not None:
    ...
    def retrieve_blended_trace(self, pattern, slot):
        """
        Returns a blend of the requested slot value from all chunks in DM that match the specified pattern,
